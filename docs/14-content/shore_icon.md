# /shore_icon 页面内容（逐字还原数据源）

> 提取自 prod-assets/pages/shore_icon.html（SSR HTML）。`<title>`: Shore Icon

## 标题区（article header, `pt-[10vh]`）

- **H1 标题**: Shore Icon
- **日期行**（`mt-1 tabular-nums text-l2!`）: Mar 01, 2022
- **描述/年份行**（分割线下方 `pt-3 text-l2!`）: 2022
...
<!-- 元素: <h1 id="发现与学习">，MDX 标题级别 1 -->

## SF Symbols

<!-- 元素: <h2 id="sf-symbols">，MDX 标题级别 2 -->
...
<!-- 元素: <h2 id="microsoft-fluent-system-icons">，MDX 标题级别 2 -->
...
<!-- 元素: <h2 id="spotify-icon-system">，MDX 标题级别 2 -->
...
<!-- 元素: <h1 id="shore-icon-system">，MDX 标题级别 1 -->
...
<!-- 元素: <h2 id="统一输出格式">，MDX 标题级别 2 -->

格式统一同样是跨端前提。我用插件导出两种格式，给研发同事评估怎么消费。

SVG Symbol 格式预览

**代码块**（语言标签: Xml, language-xml）:
